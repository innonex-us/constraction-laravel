...
<section class="mx-auto max-w-4xl px-4 py-12">
    ...
    @php($featuredImgUrl = $project->featured_image_url ?: $settings?->logo_url)
    @if($featuredImgUrl)
        <div class="mt-6 aspect-video rounded-2xl overflow-hidden border border-white/10">
            <img loading="lazy" decoding="async" fetchpriority="low" src="data:image/gif;base64,R0lGODlhAQABAAAAACw=" data-src="{{ $project->featured_image_fallback_url ?? $featuredImgUrl }}" @php($wsrc = $project->featured_image_srcset_webp ?? $project->featured_image_srcset) @if($wsrc) data-srcset="{{ $wsrc }}" sizes="(min-width:1024px) 60vw, 100vw" @endif class="w-full h-full object-cover" />
        </div>
    @endif
